...
class PrinterService {
  ...
  // Print bill
  Future<void> printBill(UnifiedDatabaseProvider dbProvider, dynamic orderId) async {
    ...
    // Generate bill text
    final billText = _generateBillText(order, items, cafeName, cafeAddress, cafePhone);
  ...
  Future<void> printKOT(UnifiedDatabaseProvider dbProvider, dynamic orderId) async {
    final order = await _orderService.getOrderById(dbProvider, orderId);
    if (order == null) {
      throw Exception('Order not found');
    }
  ...
  String _generateBillText(Order order, List<Map<String, dynamic>> items, String cafeName, String cafeAddress, String cafePhone) {
    ...
    if (cafePhone.isNotEmpty) {
      buffer.writeln('Tel: $cafePhone'.padLeft((42 + cafePhone.length + 4) ~/ 2));
    }
    buffer.writeln('=' * 42);
    buffer.writeln();
    ...
    buffer.writeln('Type: ${order.orderType == 'dine_in' ? 'Dine-In' : 'Takeaway'}');
    ...
    for (final item in items) {
      ...
      final productName = item['product_name'] as String? ?? _getProductName(item['product_id']);
    ...
    // Totals
    buffer.writeln('Subtotal:     ${NumberFormatter.formatCurrency(order.subtotal).padLeft(20)}');
    if (order.discountAmount > 0) {
      buffer.writeln('Discount:     ${NumberFormatter.formatCurrency(-order.discountAmount).padLeft(20)}');
    }
    buffer.writeln('VAT (${order.taxPercent.toStringAsFixed(0)}%): ${NumberFormatter.formatCurrency(order.taxAmount).padLeft(20)}');
    buffer.writeln('=' * 42);
    buffer.writeln('TOTAL:        ${NumberFormatter.formatCurrency(order.totalAmount).padLeft(20)}');
    buffer.writeln('=' * 42);
    buffer.writeln();
    ...
    if (order.paymentMethod != null) {
      ...
      buffer.writeln('Amount:  ${NumberFormatter.formatCurrency(order.totalAmount)}');
    }
    buffer.writeln();
  ...
  String _generateKOTText(Order order, List<Map<String, dynamic>> items, String cafeName) {
    final buffer = StringBuffer();
  ...
  Future<String> _getSetting(UnifiedDatabaseProvider dbProvider, String key, String defaultValue) async {
    try {
