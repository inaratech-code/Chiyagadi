...
class WebOfflineFirstStore {
  ...
  static Future<void> cacheProducts(List<Map<String, dynamic>> rows) async {
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(_menuProductsKey, jsonEncode(rows));
    } catch (e) {
  ...
  static Future<List<Map<String, dynamic>>> loadCachedProducts() async {
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_menuProductsKey);
      if (raw == null || raw.isEmpty) return [];
      final list = jsonDecode(raw) as List<dynamic>;
      return list.map((e) => Map<String, dynamic>.from(e as Map)).toList();
    } catch (e) {
  ...
  /// Push pending offline writes to Firestore; clears local copies on success.
