...
class DatabaseProvider with ChangeNotifier {
  ...
  Future<Database> get database async {
    if (kIsWeb) {
      throw UnsupportedError(
          'SQLite is not supported on web. Please use Android or iOS.');
    }
  ...
  Future<Database> _initDatabase() async {
    ...
    try {
      ...
      debugPrint(
          'Database: Attempting to delete corrupted database and recreate...');
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    await db.transaction((txn) async {
      ...
      debugPrint(
          'Database: Will create ${tables.length} tables, then ${indexes.length} indexes');

      // Create all tables first
      for (var i = 0; i < tables.length; i++) {
        await txn.execute(tables[i]);
        final nameMatch = RegExp(r'CREATE TABLE\s+(?:IF NOT EXISTS\s+)?(\w+)',
                caseSensitive: false)
            .firstMatch(tables[i]);
        debugPrint(
            'Database: ✓ Table ${i + 1}/${tables.length}: ${nameMatch?.group(1) ?? "?"}');
      }

      // Then create all indexes
      for (var i = 0; i < indexes.length; i++) {
        await txn.execute(indexes[i]);
        final nameMatch = RegExp(r'CREATE INDEX\s+(?:IF NOT EXISTS\s+)?(\w+)',
                caseSensitive: false)
            .firstMatch(indexes[i]);
        debugPrint(
            'Database: ✓ Index ${i + 1}/${indexes.length}: ${nameMatch?.group(1) ?? "?"}');
      }
    ...
    try {
      ...
      if (tablesExist.isNotEmpty) {
        ...
        if (!columnNames.contains('row_position')) {
          await db
              .execute('ALTER TABLE tables ADD COLUMN row_position INTEGER');
          debugPrint('Database: Added row_position column to tables');
        }
        if (!columnNames.contains('column_position')) {
          await db
              .execute('ALTER TABLE tables ADD COLUMN column_position INTEGER');
      ...
      if (categoriesExist.isNotEmpty) {
        ...
        final categoryColumnNames =
            categoryInfo.map((row) => row['name'] as String).toList();

        if (!categoryColumnNames.contains('is_locked')) {
          await db.execute(
              'ALTER TABLE categories ADD COLUMN is_locked INTEGER NOT NULL DEFAULT 0 CHECK(is_locked IN (0, 1))');
          debugPrint('Database: Added is_locked column to categories');

          // Mark default categories as locked
          await db.execute(
              "UPDATE categories SET is_locked = 1 WHERE name IN ('Food (Veg/Non Veg)', 'Cigarette', 'Beverages')");
      ...
      if (ordersExist.isNotEmpty) {
        ...
        final ordersColumnNames =
            ordersInfo.map((row) => row['name'] as String).toList();
        ...
        if (!ordersColumnNames.contains('credit_amount')) {
          await db.execute(
              'ALTER TABLE orders ADD COLUMN credit_amount REAL NOT NULL DEFAULT 0');
      ...
      if (usersExist.isNotEmpty) {
        ...
        final userColumnNames =
            usersInfo.map((row) => row['name'] as String).toList();
      ...
      if (productsExist.isNotEmpty) {
        ...
        final productColumnNames =
            productInfo.map((row) => row['name'] as String).toList();

        if (!productColumnNames.contains('is_purchasable')) {
          await db.execute(
              'ALTER TABLE products ADD COLUMN is_purchasable INTEGER NOT NULL DEFAULT 0 CHECK(is_purchasable IN (0, 1))');
          debugPrint('Database: Added is_purchasable column to products');
        }
        if (!productColumnNames.contains('is_sellable')) {
          await db.execute(
              'ALTER TABLE products ADD COLUMN is_sellable INTEGER NOT NULL DEFAULT 1 CHECK(is_sellable IN (0, 1))');
        ...
        await db.execute(
            'UPDATE products SET is_sellable = 1 WHERE is_sellable IS NULL');
        await db.execute(
            'UPDATE products SET is_purchasable = 0 WHERE is_purchasable IS NULL');
      }
      ...
      if (purchaseItemsExist.isNotEmpty) {
        ...
        if (!cols.contains('unit')) {
          await db.execute(
              "ALTER TABLE purchase_items ADD COLUMN unit TEXT NOT NULL DEFAULT 'pcs'");
...
''');
      ...
      if (purchasesExist.isNotEmpty) {
        ...
        if (!cols.contains('supplier_id')) {
          await db
              .execute('ALTER TABLE purchases ADD COLUMN supplier_id INTEGER');
        ...
        if (!cols.contains('outstanding_amount')) {
          await db.execute(
              'ALTER TABLE purchases ADD COLUMN outstanding_amount REAL NOT NULL DEFAULT 0');
    ...
    try {
      ...
      if (_database != null) {
        try {
          ...
          debugPrint(
              'Database: Error closing connection (may already be closed): $e');
        }
        _database = null;
      }
      ...
      try {
        ...
        try {
          ...
          debugPrint(
              'Database: Failed to delete database file after retry: $e2');
